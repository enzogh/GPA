...
$config = [
    ...
    'PLUGINS_SECURE_LOAD'        => [],
    'PLUGINS_SECURE_LOAD_RESULT' => [],

    'PLUGINS_ENCRYPT_FILE'        => [],
    'PLUGINS_ENCRYPT_FILE_RESULT' => [],
    ...
    if (file_exists($config['CFG']['dir'].'/'.$config['PLUGINS'][$i].'/GPA-autoloader.php')) {
        ...
        if ($GPA_PLUGINS['status'] == 'enabled') {
            if ($GPA_PLUGINS['role'] == 'classes') {
                ...
                array_push($config['PLUGINS_LOADED'], ['file_directory' => $config['PLUGINS'][$i], 'plugins_name' => $GPA_PLUGINS['name'], 'plugins_role' => $GPA_PLUGINS['role'], 'plugins_location' => $GPA_PLUGINS['location'], 'plugins_version' => $GPA_PLUGINS['version'], 'plugins_enabled' => $GPA_PLUGINS['status']]);
                ...
                if (!empty($GPA_PLUGINS['file_execute'])) {
                    ...
                    array_push($config['PLUGINS_ERROR'], ['file_directory' => $config['PLUGINS'][$i], 'message' => 'Variable executer not found']);
                }
            }
        } elseif ($GPA_PLUGINS['status'] == 'disabled' || $GPA_PLUGINS['status'] != 'enabled') {
            array_push($config['PLUGINS_DISABLED'], ['file_directory' => $config['PLUGINS'][$i], 'plugins_name' => $GPA_PLUGINS['name'], 'plugins_role' => $GPA_PLUGINS['role'], 'plugins_location' => $GPA_PLUGINS['location'], 'plugins_version' => $GPA_PLUGINS['version'], 'plugins_enabled' => $GPA_PLUGINS['status']]);
        }
    } else {
        array_push($config['PLUGINS_ERROR'], ['file_directory' => $config['PLUGINS'][$i], 'message' => 'GPA not found']);
    }
}
